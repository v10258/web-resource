(function($) {
    ...
    var _const =  {
        ...
        uiLogin: '<div class="ui-login" id="uiLogin">'
        ...
        uiArea: '<div class="ui-area" id="uiArea">'
        ...
        uiBackdrop: '<div class="ui-backdrop" id="uiBackdrop"></div>',

        uiBaiduLogin: '<div class="ui-baiduLogin">'
                        + '<div class="ui-baidu-hd">'
                        + '<h2 class="ui-dialog-title"></h2>'
                        + '<a class="ui-baidu-close" href="javascript:;">&times;</a>'
                        + '</div>'
                        + '<div class="ui-baidu-bd">'
                        + '</div>'
                        + '</div>',
    ...
    PublicModuleInit.prototype = {
        ...
        renderLogin: function() {
            $(this.opts.loginContainer).append(_const.uiLogin);
        },
        getLoginInfo: function() {
            var self = this,
                opts = self.opts,
                remoteUrl = opts.remoteUrl,
                loginSuccess = opts.loginSuccess || self.loginSuccess;
    ...
    function AreaModal(opts) {
        ...
        var method = {
            renderArea: function() {
                var htmlStr = '';
                if (!$('#uiBackdrop').length) {
                    htmlStr += _const.uiBackdrop;
                }

                if (!$('#uiArea').length) {
                    htmlStr += _const.uiArea;
                }
    ...
    function LoginInit(){
        var method = {
            renderBaidu: function() {
                var htmlStr = '';
                if (!$('#uiBackdrop').length) {
                    htmlStr += _const.uiBackdrop;
                }

                if (!$('#uiBaiduLogin').length) {
                    htmlStr += _const.uiBaiduLogin;
                }
        ...
        $('#btnBaiduLogin').on('click', function(){
            method.show();
        })
    };
    ...
    window.PublicModuleInit = PublicModuleInit;
            ...
            $('#btnBDLogin').on('click', function() {
                $('#popShow1').show();
                $('#fullbg1').show();
                if (!_parent.config.isInitBdLogin) {
                    _parent.config.isInitBdLogin = true;
                    bd_oauth.init({
            ...
            $('#btnBdLoginClose').on('click', function() {
                $('#popShow1').hide();
                $('#fullbg1').hide();
            })
